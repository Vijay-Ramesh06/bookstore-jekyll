...
## Use Case 1 â€” Build a Book Search Page
...
results.data.forEach(book => {
  console.log(`${book.title} by ${book.author.name} â€” $${book.price}`);
});
```

## Use Case 2 â€” Place an Order
...
## Use Case 3 â€” Inventory Management Dashboard
...
for book in low_stock:
    print(f"LOW STOCK: {book['title']} â€” {book['available']} remaining")
```

## Use Case 4 â€” Order Notification System with Webhooks
...
app.post('/webhooks/bookstore', (req, res) => {
  ...
  switch (type) {
    case 'order.created':
      console.log(`New order ${data.order.id} â€” $${data.order.totalAmount}`);
      sendConfirmationEmail(data.order);
      break;
    case 'order.shipped':
      console.log(`Order ${data.order.id} shipped â€” tracking: ${data.order.trackingNumber}`);
